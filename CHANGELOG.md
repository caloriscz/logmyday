...
### Added
  ...
  - MCP server: LogMyDay now speaks the Model Context Protocol at `/mcp`, so AI agents such as Claude Code can work with your data — 96 tools covering logging (`log_value` resolves a tag by name and encodes the value for its input type), tags and reference data, reminders, todo lists, day locks, scan mappings, a server-side `get_activity_summary` (totals, streaks, day/week/month buckets), event log, backup, Excel export, profile and admin; `logmyday://` resources; and three prompts (`daily_review`, `weekly_summary`, `log_by_conversation`). Destructive tools require a confirmation sentinel and report their impact first; every write is recorded in the Event Log under the new **MCP agent** type. See `docs/features/mcp.md`.
  - API keys: create read-only or read-write keys with optional expiry on the Profile page; the token is shown once, keys can be revoked, and they authenticate the MCP endpoint only. Keyed requests get their own rate-limit bucket (120/min per key).
