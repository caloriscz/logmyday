...
public class BackupService : IBackupService
{
    ...
    public async Task<BackupData> ExportDataAsync(Guid? userId = null)
    {
        ...
        try
        {
            ...
            // Export Activities with user filtering. Rows generated by a Tag Activity Relations rule
            // are not exported: they are recomputed from their sources.
    ...
    public async Task<int> ClearDataAsync(Guid? userId = null)
    {
        ...
        try
        {
            ...
            // Tag Activity Relations rules reference tags (Restrict), so they go before the tags.
            var tagRulesQuery = _context.TagRules.Include(r => r.Sources).AsQueryable();
            if (userId.HasValue)
            {
                tagRulesQuery = tagRulesQuery.Where(r => r.UserId == userId);
            }
            var tagRulesToDelete = await tagRulesQuery.ToListAsync();
            _context.TagRules.RemoveRange(tagRulesToDelete);
            recordsCleared += tagRulesToDelete.Count;
    ...
    public async Task<int> ClearUserData(Guid userId)
    {
        ...
        try
        {
            ...
            try
            {
                ...
                // Tag Activity Relations rules reference tags (Restrict), so they go before the tags.
                var userTagRules = await _context.TagRules
                    .Include(r => r.Sources)
                    .Where(r => r.UserId == userId)
                    .ToListAsync();

                _context.TagRules.RemoveRange(userTagRules);
                recordsCleared += userTagRules.Count;
