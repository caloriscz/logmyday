...
export function initializeBreathing() {
    const els = {
        toggle: document.getElementById('startToggleBtn'),
    ...
    function fmt(t) {
        return Math.max(0, Math.ceil(t)).toString();
    }
    ...
    function stopAll() {
        ...
        if (els.countdown) els.countdown.textContent = '0';
    ...
    if (els.toggle) els.toggle.addEventListener('click', handleToggle);
