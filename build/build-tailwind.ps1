...
if (Test-Path (Join-Path $repoRoot "src\ui")) {
...
if ($Clean) {
    ...
    if (Test-Path "src\ui\dist") {
        Remove-Item "src\ui\dist" -Recurse -Force
        Write-Host "  ✓ Removed src/ui/dist" -ForegroundColor Green
    }
    
    if (Test-Path "src\ui\node_modules") {
        Write-Host "  Removing node_modules (this may take a moment)..." -ForegroundColor Gray
        Remove-Item "src\ui\node_modules" -Recurse -Force
        Write-Host "  ✓ Removed src/ui/node_modules" -ForegroundColor Green
    }
...
if ($LASTEXITCODE -eq 0) {
    ...
    if (Test-Path "src\ui\dist\css\tailwind.css") {
        $cssSize = (Get-Item "src\ui\dist\css\tailwind.css").Length / 1KB
        Write-Host "    - tailwind.css: $([math]::Round($cssSize, 2)) KB" -ForegroundColor Gray
    }
    if (Test-Path "src\ui\dist\js\app.js") {
        $jsSize = (Get-Item "src\ui\dist\js\app.js").Length / 1KB
...
if ($BuildWeb) {
    ...
    dotnet build src/LogMyDay.App/LogMyDay.App.csproj
...
if ($BuildMobile) {
    ...
    dotnet build src/LogMyDay.App.Mobile/LogMyDay.App.Mobile.csproj
...
Write-Host "  • Run web app:       .\build\build-tailwind.ps1 -RunWeb" -ForegroundColor Gray
Write-Host "  • Build web app:     .\build\build-tailwind.ps1 -BuildWeb" -ForegroundColor Gray
Write-Host "  • Build mobile:      .\build\build-tailwind.ps1 -BuildMobile" -ForegroundColor Gray
Write-Host "  • Watch CSS changes: .\build\build-tailwind.ps1 -Watch" -ForegroundColor Gray
Write-Host "  • Clean build:       .\build\build-tailwind.ps1 -Clean" -ForegroundColor Gray
Write-Host ""
